...
def plot_price_history_emerge(nft_id, usd=True, save_file=False, bg_img=None, plot_floor_price=False, limit_volume=True, show_fig = True, plt_current_floor=False, subfolder=None):
    """
    ...
    non_guild = ["THE 'REGINA' ORB"]
    if nft_data['name'] in non_guild:
        name = nft_data['name']
    else:
...
#     for nft in col.get_nftId_list():
#         plot_price_history(nft,limit_volume=False, save_file=True, plt_current_floor=True, show_fig=show)
...
def plot_eth_volume(nft_list, period = [1,7,30], save_file=False, show_fig=True, file_name="eth_volume", subfolder=None):
    """
    ...
    if save_file:
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        filename = "".join(x for x in file_name if (x.isalnum() or x in "._- ")) + '.png'
