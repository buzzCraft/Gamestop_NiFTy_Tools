#
# Code modified from Historic Crypto 0.1.6 by David Woroniuk
...
class CoinbaseAPI(object):
    ...
    def retrieve_data(self):
        ...
        print("Request URL: https://api.pro.coinbase.com/products/{0}/candles?start={1}&end={2}&granularity={3}".format(
            self.ticker,
            self.start_date_string,
            self.end_date_string,
            self.granularity))
        if request_volume <= 300:
            print("Less than 300 requests")
            ...
            if response.status_code in [200, 201, 202, 203, 204]:
                ...
                data["time_index"] = pd.to_datetime(data["time"], unit='s')
                data["time"] = pd.to_datetime(data["time"], unit='s')
                data = data[data['time_index'].between(start, end)]
                data.set_index("time_index", drop=True, inplace=True)
